...
class PackageConverterTestCase(TestCase):
    ...
    def test_conversion_of_package_with_dependencies(self):
        ...
        """
        # Prepare for a bit of a weird edge case due to the (dubious)
        # deduplication of repeating words that py2deb does when generating
        # package names.
        python_debian = 'python3-python-debian' if sys.version_info[0] == 3 else 'python-debian'
        # Use a temporary directory as py2deb's repository directory so that we
        # can easily find the *.deb archive generated by py2deb.
        with TemporaryDirectory() as directory:
            # Run the conversion command.
            ...
            expected_dependencies.add(python_debian)
            ...
            expected_dependencies = [fix_name_prefix(p) for p in (
                'python-chardet', 'python-coloredlogs',
                'python-executor', 'python-humanfriendly'
            )]
            expected_dependencies.append(python_debian)
